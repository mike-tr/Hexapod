...
class IKSystem3:
    def __init__(self, L1, angle1, L2, angle2, L3, angle3):
        ...
        self.angle2 = angle2
        self.angle3 = angle3
        self.max_reach = (L1 + L2 + L3) / NORMALIZING_FACTOR 
    ...
    def angles_from_position(self, x,y,z):
        ...
        csqr =  nxsqr + z**2 
        c = math.sqrt(csqr)
        if c > (self.L2 + self.L3):
            raise ValueError(f"Target unreachable: distance {c:.2f} > max {self.L2 + self.L3:.2f}")
        
        if c < abs(self.L2 - self.L3):
            raise ValueError(f"Target too close: distance {c:.2f} < min {abs(self.L2 - self.L3):.2f}")
        ...
        return math.degrees(theta1) + self.angle1, math.degrees(theta2) + self.angle2, math.degrees(theta3) + self.angle3
